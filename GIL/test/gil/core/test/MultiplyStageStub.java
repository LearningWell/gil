...
public class MultiplyStageStub extends PipelineStage {
    ...
    public void processSignals(ByteBuffer values, SignalMetadata.DataflowDirection direction) {
        do {
            ...
            values.putInt(pos, value * _factor);
        } while (values.hasRemaining());
    }

    public Map<String, String> invokeControlCommand(Command command) {
        if (command.isCommand("FACTOR")) {
            _factor = Integer.parseInt(command.getParameters().values().iterator().next());
        }
